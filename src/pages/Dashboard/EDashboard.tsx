...
const EDashboard: React.FC = () => {
  ...
  const handlePointsRedeemedWrapper = (points: number) => { redeem(points); };
        ...
        <div className="container mx-auto relative z-10">
          <div className="flex items-center justify-between mb-4">
            <div className="flex-1 min-w-0">
              ...
              {userData?.department && (
                <p className="text-white/70 text-xs mt-0.5">Dept: {userData.department}</p>
              )}
            </div>
          ...
          {/* Stats — 3 cols mobile, 5 desktop */}
          <div className="grid grid-cols-3 md:grid-cols-5 gap-2 md:gap-3">
            <Card className="bg-white/10 border-white/20">
              <CardContent className="p-2 md:p-3 text-center text-white">
                <div className="text-lg md:text-xl font-bold">{coins}</div>
                <p className="text-xs text-white/80">Points</p>
              </CardContent>
            </Card>
            <Card className="bg-white/10 border-white/20">
              <CardContent className="p-2 md:p-3 text-center text-white">
                <div className="text-lg md:text-xl font-bold">{scansToday}</div>
            ...
            <Card className="bg-white/10 border-white/20 hidden md:block">
              <CardContent className="p-3 text-center text-white">
                <div className="text-xl font-bold">{pickupRequests.filter(r => r.status === "pending").length}</div>
          ...
          {/* Extra 2 stats on mobile only */}
          <div className="grid grid-cols-2 gap-2 mt-2 md:hidden">
            <Card className="bg-white/10 border-white/20">
              <CardContent className="p-2 text-center text-white">
                <div className="text-lg font-bold">{pickupRequests.filter(r => r.status === "pending").length}</div>
                <p className="text-xs text-white/80">Pending Pickups</p>
              </CardContent>
            </Card>
            <Card className="bg-white/10 border-white/20">
              <CardContent className="p-2 text-center text-white">
                <div className="text-lg font-bold">{Math.round((weeklyProgress / weeklyGoal) * 100)}%</div>
                <p className="text-xs text-white/80">Weekly Goal</p>
              </CardContent>
            </Card>
          </div>
          ...
          <TabsContent value="overview" className="space-y-4 md:space-y-6">
            <div>
              ...
              <div className="flex space-x-3 md:space-x-4 overflow-x-auto py-2 pb-3">
                {[
                  { icon: <QrCode className="h-6 w-6 md:h-7 md:w-7" />, label: "Generate QR", desc: isGeneratingQR ? "Generating..." : "Citizens scan to verify", onClick: handleGenerateQR },
                  { icon: <Truck className="h-6 w-6 md:h-7 md:w-7" />, label: "Track Truck", desc: "Locate vehicles", onClick: () => setShowWasteTracking(true) },
                  { icon: <Calendar className="h-6 w-6 md:h-7 md:w-7" />, label: "Schedule Pickup", desc: "Manage schedules", onClick: () => setShowSchedulePickup(true) },
                  { icon: <Calendar className="h-6 w-6 md:h-7 md:w-7" />, label: "E-Waste Day", desc: "Monthly drive", onClick: () => setShowEWasteDay(true) },
                ].map((action, i) => (
                  <Card key={i}
                ...
                <CardContent>
                  <div className="flex flex-col sm:flex-row items-center gap-4 md:gap-6">
                    <img src={generatedQR} alt="QR Code"
                      className="w-36 h-36 md:w-44 md:h-44 rounded-xl border-2 border-primary/20 shadow-md flex-shrink-0" />
                    <div className="space-y-3 w-full">
                      <div className="grid grid-cols-2 gap-2 text-sm">
                        <div className="p-2 md:p-3 bg-white dark:bg-background rounded-lg border">
                          <p className="text-muted-foreground text-xs">Employee</p>
                          <p className="font-semibold text-xs md:text-sm truncate">{user?.displayName || "—"}</p>
                        </div>
                        <div className="p-2 md:p-3 bg-white dark:bg-background rounded-lg border">
                          <p className="text-muted-foreground text-xs">Employee ID</p>
                          <p className="font-semibold text-xs md:text-sm">{userData?.employeeId || user?.uid?.slice(0, 8)}</p>
                        </div>
                        <div className="p-2 md:p-3 bg-white dark:bg-background rounded-lg border">
                      ...
                      <div className="flex flex-col sm:flex-row gap-2">
                        <Button onClick={handleDownloadQR} variant="outline" size="sm" className="flex items-center gap-2 flex-1">
                          <Download className="h-4 w-4" /> Download
                        </Button>
                        <Button onClick={handleCitizenScanned} size="sm" className="flex items-center gap-2 flex-1">
                          <CheckCircle className="h-4 w-4" /> Simulate Scan
                        </Button>
                      </div>
                      <p className="text-xs text-muted-foreground">
                        💡 "Simulate Scan" is for demo — in production, citizens scan via the app.
                      </p>
                    </div>
                  </div>
                </CardContent>
            ...
            {/* Citizen Pickup Requests — FIXED mobile overflow */}
            <Card>
              <CardHeader className="pb-2 md:pb-4">
                  ...
                  {pickupRequests.map((req) => (
                    <div key={req.id}
                      className={`p-3 md:p-4 rounded-lg border ${
                        req.status === "completed" ? "bg-green-50 border-green-200 dark:bg-green-950/20" : "bg-orange-50 border-orange-200 dark:bg-orange-950/20"
                      }`}>
                      {/* Top row: citizen + status */}
                      ...
                      <div className="flex items-center justify-between gap-2 flex-wrap">
                        ...
                        {req.status === "pending" && (
                          <Button size="sm" className="text-xs h-7 flex-shrink-0" onClick={() => handleCompletePickup(req.id)}>
                            ...
                            <span className="hidden xs:inline">Mark Done </span>(+30 pts)
                          </Button>
                        )}
                      </div>
                    </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
              ...
              <Card>
                <CardHeader className="pb-2">
                  ...
                  <CardDescription className="text-xs md:text-sm">
                    {weeklyGoal - weeklyProgress} pts away from goal!
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  ...
                  <p className="text-xs text-muted-foreground mt-2">
                    {Math.round((weeklyProgress / weeklyGoal) * 100)}% complete this week 💪
                  </p>
                </CardContent>
              </Card>
            </div>
            ...
            <Card className="border-primary/20 bg-primary/5">
              <CardHeader className="pb-2 md:pb-4">
                <CardTitle className="flex items-center gap-2 text-primary text-base md:text-lg">
                  ...
                  Your Duties as Municipal Employee
                </CardTitle>
            ...
            <div className="grid gap-4 md:gap-6 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
              <Card className="hover:shadow-lg transition-shadow">
                <CardHeader>
                  ...
                  <CardDescription>Complete the mandatory training program</CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="space-y-3">
              ...
              <Card className="hover:shadow-lg transition-shadow">
                <CardHeader>
                  ...
                  <CardDescription>Play games to reinforce knowledge</CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="space-y-3">
              ...
              <Card className="hover:shadow-lg transition-shadow">
                <CardHeader>
                  <CardTitle className="text-base md:text-lg flex items-center">
                    <Users className="mr-2 h-5 w-5" /> Employee Courses
                  </CardTitle>
                  <CardDescription>Role-specific training modules</CardDescription>
                </CardHeader>
                <CardContent>
                  ...
                  {leaderboardData.map((u, idx) => (
                    <div key={idx}
                      ...
                      <div className="flex items-center gap-2 md:gap-3">
                        <div className={`w-7 h-7 md:w-8 md:h-8 rounded-full flex items-center justify-center font-bold text-xs flex-shrink-0
        ...
        <div className="fixed inset-0 z-50 bg-black/50 flex items-start justify-center p-4 md:p-6 overflow-auto">
          <div className="bg-white w-full max-w-6xl rounded-lg shadow-xl">
            ...
            <div style={{ minHeight: 600 }}>
              <RewardsSystem onBack={() => setShowRewards(false)} onRedeem={(points: number) => handlePointsRedeemedWrapper(points)} />
            </div>
          </div>
        </div>
      )}
